...
def detectHockeyRinkEdgeByColor(image, pointColor, showDebugImage=False):
    ...
    print(pointColor)
    lower_blue = np.array([pointColor[0] - 5, max(0, pointColor[1] - 100), max(0, pointColor[2] - 100)])
    upper_blue = np.array([pointColor[0] + 5, min(255, pointColor[1] + 50), min(255, pointColor[2] + 50)])
...
def click_and_crop(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONUP:
        hsvColor = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
        finalImage = detectHockeyRinkEdgeByColor(image, pointColor=hsvColor[y][x][:], showDebugImage=True)
        cv2.imshow('final Ouput', finalImage)


# construct the argument parser and parse the arguments

# load the image, clone it, and setup the mouse callback function
filename = 'C:\\vcs\\dataset\\nhl\\image\\Bruins-Leafs Game 2 Highlights 4_14_18\\frame_0.0.jpg'
image = cv2.imread(filename)
cv2.namedWindow("image")
cv2.setMouseCallback("image", click_and_crop)
cv2.imshow("image", image)
vidoname = 'C:/vcs/dataset/nhl/video/Bruins-Leafs Game 2 Highlights 4_14_18.mp4'
